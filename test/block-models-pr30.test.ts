...
describe('PR30 carpet', () => {
  ...
  it('builds a 1px floor plate that is not full-cube', () => {
    ...
    assert.equal(model.renderBoxes[0]!.max[1], PX);
...
describe('PR30 snow layer', () => {
  ...
  it('scales Y max with layer count', () => {
    ...
    assert.equal(one.renderBoxes[0]!.max[1], 2 * PX);
...
describe('PR30 torch', () => {
  it('floor torch is upright cross; wall torch follows torch_facing_direction', () => {
    ...
    const wall = torchModel({ name: 'minecraft:torch', states: { torch_facing_direction: 'west' } });
    assert.equal(wall.renderBoxes.length, 1);
    assert.equal(wall.renderBoxes[0]!.min[0], 0);
  });
});
...
describe('PR30 cactus', () => {
  it('insets 1px on sides and is not full-cube', () => {
    ...
    assert.equal(model.renderBoxes[0]!.min[0], PX);
    assert.equal(model.renderBoxes[0]!.max[0], 1 - PX);
    assert.equal(model.isFullCube, false);
