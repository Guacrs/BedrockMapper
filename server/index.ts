...
const TEXTURES_ROOT = path.join(projectRoot, 'data', 'textures');
...
export async function startServer(config: Config, options: StartServerOptions = {}): Promise<StartedServer> {
  ...
  async function handle(request: http.IncomingMessage, response: http.ServerResponse): Promise<void> {
    ...
    // Optional Minecraft texture atlas (generated by `npm run textures:build`).
    // Missing files return 404 JSON — the 3D viewer falls back to vertex colours.
    if (pathname === '/api/textures/atlas.json' || pathname === '/api/textures/atlas.png') {
      const file = pathname.endsWith('.json') ? 'atlas.json' : 'atlas.png';
      const target = path.join(TEXTURES_ROOT, file);
      try {
        const bytes = await fs.readFile(target);
        if (file.endsWith('.png')) {
          sendPng(response, bytes, 'atlas');
        } else {
          response.writeHead(200, {
        ...
        sendJson(response, 404, {
          error: 'texture atlas not built',
          hint: 'VANILLA_SAMPLES=/path/to/bedrock-samples npm run textures:build',
        });
      }
      return;
    }
